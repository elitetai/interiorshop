...
def checkout(request, first_name, last_name, email, address, zipcode, place, phone, amount):
    ...
    for item in Cart(request):
        # item['product'] refers to the product object
...
def notify_vendor(order):
    ...
    for vendor in order.vendors.all():
        to_email = vendor.created_by.email_notify_customer
        subject = 'New order'
        ...
        html_content = render_to_string('order/email_notify_vendor.html', data)
